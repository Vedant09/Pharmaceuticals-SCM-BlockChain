...
export class ContractService {
  ...
  createOrder(orderno: string, product: string, value: string, quantity: string, delivery: string) {
    return this.OrderRegistry.deployed().then(instance => {
     return instance.createOrder(orderno, product, value, quantity, delivery, {gas: 5000000}).then(result => {
        return result;
      })
    });
  }
  ...
  getTransaction(blockNumber) {
    return this.web3.eth.getTransactionFromBlock(blockNumber);
  }

  getQr(data): Observable<Blob> {
    return this.http.get(`http://api.qrserver.com/v1/create-qr-code/?data=${data}&format=png`, { responseType: 'blob' });
  }
